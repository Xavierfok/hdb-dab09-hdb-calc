...
@st.cache_resource
def load_model():
    return joblib.load("hdb_lr_log_model1.pkl")
...
with col2:
    ...
    bto_launched = st.number_input(
        "BTO Launched (units)", min_value=0, max_value=50000, value=0, step=100,
        help="Number of BTO units launched in the area"
    )
    mop_flats = st.number_input(
        "MOP Flats (units)", min_value=0, max_value=50000, value=0, step=100,
        help="Number of flats reaching Minimum Occupation Period"
    )
...
summary = pd.DataFrame({
    "Feature": [
        ...
        "Mid Storey", "BTO Launched", "MOP Flats", "Town"
    ],
    "Value": [
        floor_area_sqft, remaining_lease_years, mrt_nearest_distance,
        mid, bto_launched, mop_flats, town
